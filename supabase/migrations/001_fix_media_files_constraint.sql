...
-- First, check for and drop any unique constraints on file_path
-- Common constraint names to check:
ALTER TABLE media_files DROP CONSTRAINT IF EXISTS media_files_file_path_key;
ALTER TABLE media_files DROP CONSTRAINT IF EXISTS media_files_file_path_unique;

-- Drop any unique indexes on file_path
DROP INDEX IF EXISTS media_files_file_path_idx;
DROP INDEX IF EXISTS media_files_file_path_key;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'media_files_backup_file_unique'
  ) THEN
